...
def test_math_erf_op(dtype, device):
    ...
    torch.testing.assert_close(z_tri, z_ref.to(device))
...
def test_abs_fp8(in_dtype, device):
    if is_hip():
        ...
        if in_dtype == tl.float8e4b15 and is_musa_ph1():
            pytest.skip("float8e4nv not supported on PH arch")
...
def test_tensor_atomic_cas(sem, size, dtype_str, num_ctas, device):
    ...
    X[1::2] = 1
    if device == "musa" and torch_dtype is torch.uint64:
        # torch_musa uint64 clone/copy_ currently errors; keep the oracle on CPU.
        Y = X.to('cpu').clone()
    else:
        Y = X.clone()
    Y[0::2] = 2
    ...
    assert torch.equal(X.to('cpu'), Y.to('cpu'))
...
def test_atomic_unsupported_type(dtype_str, device):
    ...
    I = torch.zeros((1, ), device='cpu', dtype=getattr(torch, dtype_str)).to(device)
    O = torch.zeros((1, ), device='cpu', dtype=getattr(torch, dtype_str)).to(device)
...
def convert_float_to_float32(fp: torch.tensor, dtype=None):
    ...
    if dtype in [tl.float8e4nv, tl.float8e4b15]:
        ...
        if dtype == tl.float8e4nv and fp.device.type == "musa":
            fp_bits = fp.to(torch.int16) & 0xFF
            output[fp_bits == 0x7F] = torch.nan
            output[fp_bits == 0xFF] = torch.nan
        else:
...
def test_histogram(M, N, device):
    ...
    x = torch.randint(0, N, (M, ), device='cpu', dtype=torch.int32).to(device)
...
def test_no_rematerialization_op(device):
    ...
    data_dim = 64
    torch.manual_seed(0)
    input_data = torch.randn((data_len, data_dim), dtype=torch.float32, device='cpu').to(device)
    sum_output = torch.full((data_len, ), -1, dtype=torch.float32, device='cpu').to(device)
    out_1 = torch.full((data_len, 2), -1, dtype=torch.float32, device='cpu').to(device)
...
def test_permute(dtype_str, shape, perm, num_ctas, device):
    check_type_supported(dtype_str, device)  # bfloat16 on cc < 80 will not be tested
    if dtype_str == "float8e4b15" and (is_musa() or is_hip() or
...
def test_trans_2d(dtype_str, shape, perm, device):
    ...
    actual = torch.zeros(expected.shape, dtype=getattr(torch, dtype_str), device='cpu').to(device)
...
def test_scaled_dot(M, N, K, col_a, col_b, rhs_scale, mxfp_type, normal_type, num_warps, mma, kpack, device):
    ...
    def make_finite(x, dtype):
        ...
        if is_musa():
            finite = (torch.arange(x.numel(), device=device, dtype=torch.int32) % mask).to(torch.uint8).reshape_as(x)
        else:
...
def test_full(dtype_str, shape, device):
    ...
    out_static = torch.zeros((128), dtype=dtype, device='cpu').to(device)
    kernel_static_patched[(1, )](out_static)
    assert torch.all(out_static.to('cpu') == 2)

    kernel_dynamic_patched = patch_kernel(kernel_dynamic, {'SHAPE': str(list(shape))})
    out_dynamic = torch.zeros((128), dtype=dtype, device='cpu').to(device)
    kernel_dynamic_patched[(1, )](out_dynamic, 2, getattr(triton.language, dtype_str))
    assert torch.all(out_dynamic.to('cpu') == 2)
...
def test_arange(start, num_ctas, device):
    ...
    z_ref = torch.arange(start, BLOCK + start, dtype=torch.int32, device='cpu').to(device)
...
def test_poison_return(device):
    ...
    if not (is_hip() or is_xpu() or is_musa()):
...
def test_num_threads(device):
    ...
    if is_musa():
        pytest.skip("test_num_threads is not supported in MUSA")
...
def test_ptx_cast(dtype_str, device):
    if is_musa():
        pytest.skip("test_ptx_cast no longer needs to be tested in MUSA")
...
_DOT_MAX_NUM_IMPRECISE_ACC_IN_TYPES = ["float8e5", "float8e4nv"]
if is_hip():
    _DOT_MAX_NUM_IMPRECISE_ACC_IN_TYPES += ["float8e5b16", "float8e4b8"]
elif is_cuda():
    _DOT_MAX_NUM_IMPRECISE_ACC_IN_TYPES += ["float8e4b15"]
...
@pytest.mark.parametrize(
    "in_type_str",
    _DOT_MAX_NUM_IMPRECISE_ACC_IN_TYPES,
)
...
def test_num_ctas_pre_sm90(device, fresh_knobs):
    if not is_cuda() and not is_hip() and not is_musa():
        pytest.skip("Only supported on CUDA, HIP, and MUSA")
    ...
    if is_cuda():
        ...
        arch = "ph1"
        msg = r"num_ctas > 1 requires MUSA cluster launch support"
...
def test_zero_strided_tensors(device):
    ...
    x = torch.zeros((2, 2, 1), device='cpu').to(device)
    c_dim = 3
    x = x.expand((2, 2, c_dim))

    a, b, c = x.shape
    grid = (a, b, c)
    with torch.musa.device(x.device.index):
