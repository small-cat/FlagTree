...
SmallVector<unsigned> getElemsPerThread(Type type) {
  ...
  if (auto vecType = dyn_cast<VectorType>(type)) {
    return SmallVector<unsigned>(vecType.getShape().begin(),
...
Attribute MUSAWmmaEncodingAttr::parse(AsmParser &parser, Type type) {
  if (parser.parseLess().failed())
    return {};
  DictionaryAttr dict;
  if (parser.parseAttribute(dict).failed())
  ...
  return parser.getChecked<MUSAWmmaEncodingAttr>(
      parser.getContext(), versionMajor, versionMinor, warpsPerCTA, *CGALayout,
      instrShape);
}

void MUSAWmmaEncodingAttr::print(AsmPrinter &printer) const {
  printer << "<{"
          << "versionMajor = " << getVersionMajor()
...
LogicalResult MUSAWmmaEncodingAttr::verify(
  ...
  if (warpsPerCTA.empty())
    return emitError() << "warpsPerCTA must be non-empty";
  if (warpsPerCTA.size() > 3)
...
Attribute MUSASqmmaEncodingAttr::parse(AsmParser &parser, Type type) {
  if (parser.parseLess().failed())
    return {};
  DictionaryAttr dict;
  if (parser.parseAttribute(dict).failed())
  ...
  return parser.getChecked<MUSASqmmaEncodingAttr>(
      parser.getContext(), versionMajor, versionMinor, warpsPerCTA, *CGALayout,
      instrShape);
}

void MUSASqmmaEncodingAttr::print(AsmPrinter &printer) const {
  printer << "<{"
          << "versionMajor = " << getVersionMajor()
          << ", versionMinor = " << getVersionMinor() << ", warpsPerCTA = ["
          << ArrayRef(getWarpsPerCTA()) << "]";
...
LogicalResult MUSASqmmaEncodingAttr::verify(
    function_ref<mlir::InFlightDiagnostic()> emitError, unsigned versionMajor,
    unsigned versionMinor, ArrayRef<unsigned> warpsPerCTA,
    CGAEncodingAttr cgaLayout, ArrayRef<unsigned> instrShape) {
  if (warpsPerCTA.empty())
  ...
  for (unsigned idx = 1; idx < instrShape.size(); ++idx) {
    if (instrShape[idx] == 0 || (instrShape[idx] % 8) != 0)
      return emitError()
...
SmallVector<unsigned>
MUSAWmmaEncodingAttr::getRepOrderForOperand(int opIdx) const {
  return getOrderForDotOperand(opIdx, getRank(), /*kContig*/ true);
}

SmallVector<unsigned> MUSASqmmaEncodingAttr::getRepOrder() const {
  return getMatrixOrder(getRank(), /*rowMajor*/ true);
}

SmallVector<unsigned>
MUSASqmmaEncodingAttr::getRepOrderForOperand(int opIdx) const {
  return getOrderForDotOperand(opIdx, getRank(), /*kContig*/ true);
}

SmallVector<unsigned>
MUSASqmmaEncodingAttr::getElemsPerThread(ArrayRef<int64_t> shape) const {
  unsigned rank = shape.size();
  ...
  unsigned squadsPerCTA = std::max(1u, warpsPerCTA[0] / 4);

  unsigned repM = ceilDiv(shapePerCTA[rank - 2], instrMNK[0] * squadsPerCTA);
  unsigned repN = ceilDiv(shapePerCTA[rank - 1], instrMNK[1] * warpsPerCTA[1]);
...
unsigned
MUSASqmmaEncodingAttr::getTotalElemsPerThread(ArrayRef<int64_t> shape) const {
  return product(getElemsPerThread(shape));
}

static SwizzledSharedEncodingAttr
composeMusaSharedLayout(MLIRContext *ctx, CGAEncodingAttr cgaLayout,
                        int operandIdx, ArrayRef<int64_t> operandShape,
                        ArrayRef<unsigned> sharedOrder, unsigned elemBitWidth,
                        bool needTrans) {
  // Determine whether the operand is laid out as MN-major (transpose) or
  // K-major (non-transpose). For dot operands, row-major means the last
  // logical dim is contiguous.
  if (sharedOrder.empty())
    return SwizzledSharedEncodingAttr::get(ctx, 1, 1, 1, sharedOrder,
                                           cgaLayout);

  unsigned rank = sharedOrder.size();
  bool isRowMajor = sharedOrder[0] == (rank - 1);
  if (needTrans && rank >= 2)
    isRowMajor = !isRowMajor;

  bool isMNMajor =
      ((operandIdx == 0) && !isRowMajor) || ((operandIdx == 1) && isRowMajor);

  auto shapePerCTA = getShapePerCTA(cgaLayout.getCTASplitNum(), operandShape);
  if (sharedOrder[0] >= shapePerCTA.size())
    return SwizzledSharedEncodingAttr::get(ctx, 1, 1, 1, sharedOrder,
                                           cgaLayout);

  unsigned elemBytes = std::max(1u, elemBitWidth / 8);
  unsigned sgBytes = 16;
  if (isMNMajor) {
    if (elemBytes == 2) {
      sgBytes = 32;
  ...
  // For non-pipelined SQMMA operands, choose the canonical swizzled_shared
  // layout whose LinearLayout is exactly representable by the PH1 TME
  // SG/SS/SL tuple that both descriptor landing and local_alloc lowering use.
  constexpr int64_t kSwizzleLineBytes = 256;
  int64_t leadingWidthBytes = shapePerCTA[sharedOrder[0]] * elemBytes;
  if (leadingWidthBytes <= 0)
    return SwizzledSharedEncodingAttr::get(ctx, 1, 1, 1, sharedOrder,
                                           cgaLayout);

  unsigned vec = std::max(1u, sgBytes / elemBytes);
  unsigned perPhase = 1;
  unsigned maxPhase =
      std::max(1u, static_cast<unsigned>(kSwizzleLineBytes / sgBytes));

  if (leadingWidthBytes >= kSwizzleLineBytes) {
    if ((leadingWidthBytes % kSwizzleLineBytes) != 0)
      return SwizzledSharedEncodingAttr::get(ctx, 1, 1, 1, sharedOrder,
                                             cgaLayout);
    int64_t factor = leadingWidthBytes / kSwizzleLineBytes;
    if (!llvm::isPowerOf2_64(static_cast<uint64_t>(factor)))
      return SwizzledSharedEncodingAttr::get(ctx, 1, 1, 1, sharedOrder,
                                             cgaLayout);
    int64_t vecElems = factor * static_cast<int64_t>(sgBytes / elemBytes);
    int64_t phases =
        kSwizzleLineBytes / (factor * static_cast<int64_t>(sgBytes));
    if (vecElems <= 0 || phases <= 0)
      return SwizzledSharedEncodingAttr::get(ctx, 1, 1, 1, sharedOrder,
                                             cgaLayout);
    vec = static_cast<unsigned>(vecElems);
    perPhase = 1;
    maxPhase = static_cast<unsigned>(phases);
  } else {
    if ((kSwizzleLineBytes % leadingWidthBytes) != 0)
      return SwizzledSharedEncodingAttr::get(ctx, 1, 1, 1, sharedOrder,
                                             cgaLayout);
    int64_t ratio = kSwizzleLineBytes / leadingWidthBytes;
    if (!llvm::isPowerOf2_64(static_cast<uint64_t>(ratio)))
      return SwizzledSharedEncodingAttr::get(ctx, 1, 1, 1, sharedOrder,
                                             cgaLayout);
    perPhase = static_cast<unsigned>(ratio);
  }

  return SwizzledSharedEncodingAttr::get(ctx, vec, perPhase, maxPhase,
                                         sharedOrder, cgaLayout);
}

SwizzledSharedEncodingAttr MUSAWmmaEncodingAttr::composeSharedLayoutForOperand(
    CGAEncodingAttr cgaLayout, int operandIdx, ArrayRef<int64_t> operandShape,
  ...
  (void)elemBitWidth;
  (void)needTrans;
  // Keep WMMA operand shared layout consistent with the 3.2 PH1 path:
  // canonical non-swizzled shared tiles for both A/B operands.
  return SwizzledSharedEncodingAttr::get(getContext(), /*vec=*/1,
                                         /*perPhase=*/1, /*maxPhase=*/1,
                                         sharedOrder, cgaLayout);
}

SwizzledSharedEncodingAttr MUSASqmmaEncodingAttr::composeSharedLayoutForOperand(
    CGAEncodingAttr cgaLayout, int operandIdx, ArrayRef<int64_t> operandShape,
    ArrayRef<unsigned> sharedOrder, unsigned kWidth, unsigned elemBitWidth,
    bool needTrans) const {
  (void)kWidth;

  // Triton 3.2 MUSA pipelined SQMMA local allocs (shape rank includes the
  // stage dimension) used MMAv3-style shared swizzle selection based on the
  // contiguous byte width. Keep that behavior for rank-expanded operands.
  if (operandShape.size() > sharedOrder.size() && elemBitWidth > 0) {
    auto shapePerCTA = getShapePerCTA(cgaLayout.getCTASplitNum(), operandShape);
    int64_t contigBytes = shapePerCTA[sharedOrder[0]] * elemBitWidth / 8;
    int perPhase = 1;
    int maxPhase = 1;
    if (contigBytes >= 128 && contigBytes % 128 == 0) {
      perPhase = 1;
      maxPhase = 8;
    } else if (contigBytes >= 64 && contigBytes % 64 == 0) {
      perPhase = 2;
      maxPhase = 4;
    } else if (contigBytes >= 32 && contigBytes % 32 == 0) {
      perPhase = 4;
      maxPhase = 2;
    } else if (contigBytes >= 16 && contigBytes % 16 == 0) {
      perPhase = 8;
      maxPhase = 1;
    }
    unsigned vec = std::max(1u, 128u / elemBitWidth);
    return SwizzledSharedEncodingAttr::get(getContext(), vec, perPhase,
                                           maxPhase, sharedOrder, cgaLayout);
  }

  return composeMusaSharedLayout(getContext(), cgaLayout, operandIdx,
                                 operandShape, sharedOrder, elemBitWidth,
                                 needTrans);
}
...
LogicalResult DotOperandEncodingAttr::verify(
  ...
  if (auto parentAttr = mlir::dyn_cast<MUSAWmmaEncodingAttr>(parent)) {
    if (kWidth != 0)
      return emitError() << "ttg.dot_op kWidth parameter is not supported "
                            "for MUSA WMMA parent";
    (void)parentAttr;
    return success();
  }
...
struct TritonGPUInferLayoutInterface
  ...
  inferDotOpEncoding(Attribute operandEncoding, unsigned opIdx,
    ...
    } else if (mlir::isa<MUSASqmmaEncodingAttr>(retEncoding)) {
      // SQMMA operands cross an explicit shared-memory or dot-operand
      // boundary. Do not accept raw MMA accumulator/fragment layouts here:
      // chained dots must first re-enter a logical tensor/shared descriptor
      // layout before they can be staged as the next operand.
      if (mlir::isa<SharedEncodingTrait, SharedLinearEncodingAttr>(
              operandEncoding))
        return success();
      auto dotOpEnc = mlir::dyn_cast<DotOperandEncodingAttr>(operandEncoding);
      if (!dotOpEnc)
        return emitOptionalError(
            location, "unexpected operand layout for MUSASqmmaEncodingAttr");
  ...
  verifyDotOpEncodingCompatibility(Operation *op, Attribute operandEncodingA,
    ...
    auto dotOp = dyn_cast<triton::DotOpInterface>(op);
    if (!dotOp)
      return op->emitError(
          "expected a dot-like operation for encoding compatibility checks");
    auto resEnc = cast<triton::gpu::TensorOrMemDesc>(dotOp.getD().getType())
                      .getEncoding();
    ...
    if (mmaAEncoding || mmaBEncoding || mmaResEncoding) {
      ...
      auto ttDot = dyn_cast<DotOp>(op);
      if (!ttDot)
        return op->emitError(
            "expected tt.dot when verifying Nvidia MMA operand support");
      if (!supportMMA(ttDot, mmaResEncoding.getVersionMajor()))
...
struct TritonGPUVerifyTensorLayoutInterface
  ...
  LogicalResult verifyTensorLayout(
    ...
    bool allowMusaWmmaDotWarpSubset = false;
    if (layoutWarpsPerCTA != *moduleWarpsPerCTA) {
      if (auto dotEnc = dyn_cast<DotOperandEncodingAttr>(layout)) {
        if (auto musaWmma =
                dyn_cast<MUSAWmmaEncodingAttr>(dotEnc.getParent())) {
          // PH1 WMMA dot operands can intentionally model only a subset of CTA
          // warps along a single operand axis (e.g. B operand with tileN=1).
          // Allow this as long as it evenly partitions the CTA warp count.
          if (musaWmma.isPH1() && layoutWarpsPerCTA > 0 &&
              (*moduleWarpsPerCTA % layoutWarpsPerCTA) == 0) {
            allowMusaWmmaDotWarpSubset = true;
          }
        }
      }
    }
    if (!allowMusaWmmaDotWarpSubset &&
