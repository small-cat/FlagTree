...
// triton_gcu400_module.cpp -- Thin pybind11 wrapper around
// lib_triton_gcu400_core.so's C ABI.
...
uintptr_t pipelineCreate() {
  auto h = gcu400_pipeline_create();
  if (!h)
    throw std::runtime_error("gcu400_pipeline_create failed");
  return reinterpret_cast<uintptr_t>(h);
}

void pipelineAddPass(uintptr_t h, const std::string &name,
                     const std::string &options) {
  gcu400_pipeline_add_pass(reinterpret_cast<Gcu400Pipeline>(h), name.c_str(),
                           options.c_str());
}

std::string pipelineRun(uintptr_t h, const std::string &input) {
  Gcu400String s = gcu400_pipeline_run(reinterpret_cast<Gcu400Pipeline>(h),
                                       input.data(), input.size());
  if (!s.data) {
    std::string err =
        gcu400_pipeline_last_error(reinterpret_cast<Gcu400Pipeline>(h));
    throw std::runtime_error("Pipeline run failed: " + err);
  }
  std::string result(s.data, s.len);
  gcu400_string_free(s);
  return result;
}

void pipelineDestroy(uintptr_t h) {
  if (h)
    gcu400_pipeline_destroy(reinterpret_cast<Gcu400Pipeline>(h));
}
...
std::string runOpt(const std::string &input,
  ...
  Gcu400String s = gcu400_run_opt(input.data(), input.size(), cargs.data(),
                                  static_cast<int>(cargs.size()));
  if (!s.data)
    throw std::runtime_error("gcu400_run_opt failed (pass pipeline error)");
  std::string result(s.data, s.len);
  gcu400_string_free(s);
  return result;
}
...
PYBIND11_MODULE(_triton_gcu400, m) {
  m.doc() = "GCU400 in-process MLIR opt pipeline (pybind11 thin binding)";
