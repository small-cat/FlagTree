...
#define MLS_DS_FLAGS_PACK(elem3, row3, col3, alt2)                             \
  (((elem3) & 0xFF) | (((row3) & 0xFF) << 8) | (((col3) & 0xFF) << 16) |       \
   (((alt2) & 0xFF) << 24))
...
public:
  static FailureOr<MlsInsn> selectOrGetMlsInsn(
      unsigned nonKTile, unsigned kTile, unsigned elemBitWidth, unsigned opIdx,
      bool kMajor,
  ...
  unsigned getMajorDimIndex() {
    return attr.opIdx == 0 ? attr.kMajor : !attr.kMajor;
  }
  unsigned getDotLayoutKWidth() {
    return attr.elemBitWidth == 16 ? 4 : (attr.elemBitWidth == 8 ? 8 : 8);
  }
  SmallVector<unsigned> getMlsTile() {
    return SmallVector<unsigned>(attr.mlsTile.begin(), attr.mlsTile.end());
  }
  unsigned getMlsNonKTile() {
    return attr.opIdx == 0 ? attr.mlsTile[0] : attr.mlsTile[1];
  }
  unsigned getMlsKTile() {
    return attr.opIdx == 0 ? attr.mlsTile[1] : attr.mlsTile[0];
  }
