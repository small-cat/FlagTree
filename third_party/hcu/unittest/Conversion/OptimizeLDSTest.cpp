...
#include "third_party/hcu/lib/TritonHCUGPUToLLVM/OptimizeLDSUtility.h"
...
TEST(OptimizeLDSUtility, factorizePowerOf2) {
  ...
  numwarps = 4;
  rank = 1;
  auto output1 = triton::HCU::factorizePowerOf2(numwarps, rank);
  ASSERT_EQ(output1.size(), 1);
  ASSERT_EQ(output1[0][0], numwarps);
  // check rank=2 generation
  numwarps = 8;
  rank = 2;
  auto output2 = triton::HCU::factorizePowerOf2(numwarps, rank);
  ...
  numwarps = 8;
  rank = 3;
  auto output3 = triton::HCU::factorizePowerOf2(numwarps, rank);
