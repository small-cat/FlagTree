...
#include "third_party/hcu/include/TritonHCUGPUToLLVM/GCNAsmFormat.h"
...
Value TargetInfo::clock(ConversionPatternRewriter &rewriter, Location loc,
  ...
  // NV has both a 32 bit and 64 bit clock intrinsic. On HCU we only have
...
int TargetInfo::getIndexPtrAddrSpace() const {
  ...
  // address space for HCU GPUs. See detail discussion:
  // https://llvm.org/docs/HCUGPUUsage.html#address-spaces
  return 5;
}
