#ifndef TRITON_THIRD_PARTY_HCU_LIB_TRITONHCUGPUTOLLVM_ASYNCUTILITY_H_
#define TRITON_THIRD_PARTY_HCU_LIB_TRITONHCUGPUTOLLVM_ASYNCUTILITY_H_
...
// Walks the module and adds a LocalBarrier after any hcug.async_wait if there
// is not already a barrier following it. This mimicks what Member does for
// common async wait operations and avoids HCU specific modifications to Membar.
...
// Annotates LocalLoadOps with ttg.hcug.syncedByAsyncWait=true if they are
...
//   %token1 = ttg.async_copy_global_to_local/hcug.buffer_load_to_local
...
//   1) "hcug.AsyncCopies" will contain all AsyncCopy ops
//   2) "hcug.LocalLoad" will contain all LocalLoads manually synchronized via
//      AsyncWait
...
//  - Attaches "hcug.LocalLoad" alias scope to llLoadOp
//  - Attaches "hcug.AsyncCopies" as *non* alias scope to llLoadOp
