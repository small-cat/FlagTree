...
static void lowerHCUAbarrierArriveOp(ROCDL::HCUAbarrierArriveOp bar,
                                     unsigned barIdx, unsigned warpStartId,
                                     unsigned numWarps) {
...
static void rewritePartitionRegions(WarpSpecializeOp ws,
  ...
  for (auto [idx, partition] : llvm::enumerate(ws.getPartitionRegions())) {
    ...
    // For the first partition, insert an ebarrier with id
    // kAbarrierInvBarrierIdx before the earliest HCUAbarrierInvOp in the
    // region, if any.
    if (idx == 0 || idx == 2) {
...
lowerWarpSpecialize(LLVM::LLVMFuncOp func, const HCU::TargetInfo &targetInfo,
                    int waspNumLoadWarps, int waspNumMmaWarps, bool wdraEnabled,
                    int wdraNumLoadRegs, int wdraNumMmaRegsMain,
  ...
  // Cases values initialized to warp ids from 0 to the number of warps in the
  // first mma partition.
  auto caseValues = llvm::to_vector(llvm::map_range(
      llvm::seq<int8_t>(mmaNumWarps), [](int8_t i) { return APInt(8, i); }));
  ...
  // Set the rest target blocks for the switch op.
  {
    ...
    for (auto [idx, tuple] :
         llvm::enumerate(llvm::zip(*firstWsOp.getWarpGroupStartIds(),
                                   firstWsOp.getPartitionNumWarps(),
                                   firstWsOp.getPartitionRegions()))) {
      ...
      for (auto i : llvm::seq(numWarps)) {
        Block *targetBlock =
            wdraEnabled ? vgprSettingBlock : &partition->front();
  ...
  b.create<LLVM::SwitchOp>(wid, defaultBlock, ValueRange(), caseValues,
                           switchBlocks,
    ...
    for (Block &b : region->getBlocks()) {
      for (Operation &op : b) {
        if (auto ret = dyn_cast<LLVM::ReturnOp>(&op)) {
          ...
          std::string asmStr =
              "; mma_epilogue_partition_" + std::to_string(partitionId);
          auto asmDialect =
          ...
          markerBuilder.create<LLVM::InlineAsmOp>(
              ...
              "",           // constraints
              /*hasSideEffects=*/true,
              /*isAlignStack=*/false, LLVM::TailCallKind::None, asmDialect,
              ArrayAttr::get(ctx, {}));
...
struct HCUGPUConvertWarpSpecializeToLLVM
  ...
  explicit HCUGPUConvertWarpSpecializeToLLVM(
      StringRef targetArch, int waspNumLoadWarps, int waspNumMmaWarps,
      bool wdraEnabled, int wdraNumLoadRegs, int wdraNumMmaRegsMain,
      int wdraNumMmaRegsTail) {
  ...
  void runOnOperation() override {
    ...
    if (targetInfo.getGPUKind() != llvm::AMDGPU::GK_GFX946) {
      mod.emitError("wasp unsupported target: '")
          << this->arch.getValue() << "'";
    ...
    for (LLVM::LLVMFuncOp kernel : kernels)
      if (failed(lowerWarpSpecialize(
              kernel, targetInfo, this->waspNumLoadWarps, this->waspNumMmaWarps,
              this->wdraEnabled, this->wdraNumLoadRegs,
              this->wdraNumMmaRegsMain, this->wdraNumMmaRegsTail)))
        return signalPassFailure();

    // Convert GPU dialect to ROCDL dialect
    FailureOr<mlir::amdgpu::Chipset> maybeChipset =
        mlir::amdgpu::Chipset::parse(this->arch.getValue());
    ...
    if (failed(
            applyPartialConversion(mod, gpuTarget, std::move(gpuPatterns)))) {
      return signalPassFailure();
    }

    // Elide canceling casts: unrealized_conversion_cast (A -> B) followed by
    // arith.index_cast (B -> A) can be removed.
    SmallVector<std::pair<UnrealizedConversionCastOp, arith::IndexCastOp>>
        cancelingPairs;
...
createHCUGPUConvertWarpSpecializeToLLVM(StringRef targetArch,
                                        ...
                                        int wdraNumMmaRegsTail) {
  return std::make_unique<HCUGPUConvertWarpSpecializeToLLVM>(
      targetArch, waspNumLoadWarps, waspNumMmaWarps, wdraEnabled,
