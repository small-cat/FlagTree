...
struct MlsInsnGroupSelectKeyInfo
  ...
  static inline bool isEqual(const MlsInsnGroupSelectKey &lhs,
    ...
    return lhs.nonKTile == rhs.nonKTile && lhs.kTile == rhs.kTile &&
           ...
           lhs.elemBitWidth == rhs.elemBitWidth && lhs.version == rhs.version &&
           lhs.alt2Kind == rhs.alt2Kind;
  }
  ...
  static unsigned getHashValue(const MlsInsnGroupSelectKey &key) {
    ...
    auto transposeHash =
        llvm::detail::combineHashValue(nonKTileHash, key.transpose);
    auto elemBitWidthHash =
        llvm::detail::combineHashValue(transposeHash, key.elemBitWidth);
    auto alt2KindHash = cast_as_underlying2(key.alt2Kind);
    auto verHash =
        llvm::detail::combineHashValue(elemBitWidthHash, alt2KindHash);
...
auto getMlsInsnGroupAttrMap = []() -> const MlsInsnGroupMap & {
  static MlsInsnGroupMap MlsInsnMap{
      // b16:
      // matrix_load_32x16_b16: AT/BN(transpose = 1), interleave none,  mnk
      // required least [16, 16, 32]
      {{16, 32, true, 16, MlsInterleaveKind::InterleaveNone, 1},
       {{16,
         32,
         ROCDL::hcu_matrix_load_32X16_b16::getOperationName(),
         {0, 0, 0, 0}},
        {16,
         32,
         ROCDL::hcu_ds_read_matrix_trans_format::getOperationName(),
         MLS_DS_FLAGS_PACK(2, 2, 1, 0),
         {0, 0, 0, 0}}}},

      // matrix_load_32x16_b16: AN/BT(transpose = 0), interleave none,  mnk
      // required least [32, 32, 16]
      {{32, 16, false, 16, MlsInterleaveKind::InterleaveNone, 1},
       {{32,
        ...
        {32,
         16,
         ROCDL::hcu_ds_read_matrix_format::getOperationName(),
         MLS_DS_FLAGS_PACK(2, 2, 1, 0),
         {0, 0, 0, 0}}}},

      // matrix_load_64x16_b16: AT/BN(transpose = 1), interleave none,  mnk
      // required least [16, 16, 64]
      {{16, 64, true, 16, MlsInterleaveKind::InterleaveNone, 1},
       {{16,
         64,
         ROCDL::hcu_matrix_load_64X16_b16::getOperationName(),
         {0, 0, 0, 0}},
        {16,
         32,
         ROCDL::hcu_ds_read_matrix_trans_format::getOperationName(),
      ...
      // matrix_load_64x16_b16: AN/BT(transpose = 0), interleave none,  mnk
      // required least [64, 64, 16]
      ...
      // b8:
      // matrix_load_64x16_b8: AT/BN(transpose = 1), interleave none,  mnk
      // required least [16, 16, 64]
      {{16, 64, true, 8, MlsInterleaveKind::InterleaveNone, 1},
       {{16,
         64,
         ROCDL::hcu_matrix_load_64X16_b8::getOperationName(),
         {0, 0, 0, 0}},
        {16,
         64,
         ROCDL::hcu_ds_read_matrix_trans_format::getOperationName(),
         MLS_DS_FLAGS_PACK(1, 3, 1, 0),
         {0, 0, 0, 0}}}},
      // matrix_load_64x16_b8: AN/BT(transpose = 0), interleave none,  mnk
      // required least [64, 64, 16]
      {{64, 32, false, 8, MlsInterleaveKind::InterleaveNone, 1},
       {{64,
        ...
        {32,
         32,
         ROCDL::hcu_ds_read_matrix_format::getOperationName(),
         MLS_DS_FLAGS_PACK(1, 2, 2, 0),
         {0, 32, 0, 0}}}},
      // matrix_load_128x16_b8: AT/BN(transpose = 1), interleave none,  mnk
      // required least [16, 16, 128]
      {{16, 128, true, 8, MlsInterleaveKind::InterleaveNone, 1},
       {{16,
         128,
         ROCDL::hcu_matrix_load_128X16_b8::getOperationName(),
         {0, 0, 0, 0}},
        {16,
         64,
         ROCDL::hcu_ds_read_matrix_trans_format::getOperationName(),
         MLS_DS_FLAGS_PACK(1, 3, 1, 0),
         {0, 1024, 0, 0}}}},
      // matrix_load_128x16_b8: AN/BT(transpose = 0), interleave none,  mnk
      // required least [128, 128, 32]
...
FailureOr<MlsInsn>
MlsInsn::selectOrGetMlsInsn(unsigned nonKTile, unsigned kTile,
                            unsigned elemBitWidth, unsigned opIdx, bool kMajor,
  ...
  std::array<unsigned, 2> order =
      opIdx == 0 ? (kMajor ? std::array<unsigned, 2>{1, 0}
  ...
  assert(oAttr.interleaveKind ==
         static_cast<MlsInterleaveKind>(
             MLS_DS_FLAGS_GET_ALT2(iAttr.dsInfo.flags)));

  if (opIdx == 0) {
    oAttr.mlInsn.instrShape = {iAttr.mlInfo.instNonKDim, iAttr.mlInfo.instKDim};
    oAttr.mlInsn.instrsPerWarp = {nonKTile / iAttr.mlInfo.instNonKDim,
                                  kTile / iAttr.mlInfo.instKDim};
    ...
    oAttr.dsInsn.instrsPerWarp = {nonKTile / iAttr.dsInfo.instNonKDim,
                                  kTile / iAttr.dsInfo.instKDim};
    oAttr.dsInsn.instrOrder =
        order; // ds_read_matrix instrOrder is always for tensor order.
    ...
    oAttr.mlInsn.instrsPerWarp = {kTile / iAttr.mlInfo.instKDim,
                                  nonKTile / iAttr.mlInfo.instNonKDim};
    oAttr.mlInsn.instrOrder = order;
    oAttr.mlInsn.insn = iAttr.mlInfo.insn;
    oAttr.mlInsn.dsByteOffsets = iAttr.mlInfo.dsByteOffsets;

    oAttr.dsInsn.instrShape = {iAttr.dsInfo.instKDim, iAttr.dsInfo.instNonKDim};
    oAttr.dsInsn.instrsPerWarp = {kTile / iAttr.dsInfo.instKDim,
                                  nonKTile / iAttr.dsInfo.instNonKDim};
  ...
  // Note: current matrix_load tile seems only exist [1, 1], [xxx, 1], [1, xxx]
  // and no [xxx, xxx] case so cur instrOrder set to tensor order now.
  assert(oAttr.mlInsn.instrsPerWarp[0] == 1 ||
         oAttr.mlInsn.instrsPerWarp[1] == 1);

  unsigned dsInstsPerWarp =
      oAttr.dsInsn.instrsPerWarp[0] * oAttr.dsInsn.instrsPerWarp[1];
  unsigned mlInstsPerWarp =
      oAttr.mlInsn.instrsPerWarp[0] * oAttr.mlInsn.instrsPerWarp[1];
  assert(dsInstsPerWarp <= MLS_INST_DS_CNT_MAX &&
         mlInstsPerWarp <= MLS_INST_DS_CNT_MAX);
