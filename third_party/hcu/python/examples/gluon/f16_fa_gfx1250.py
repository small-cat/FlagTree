...
class AttentionConfig:
    ...
    def __init__(self, SEQLEN_Q, SEQLEN_K, HEAD_SZ, BLOCK_M, BLOCK_N, NUM_BUFFERS):
        ...
        self.qk_layout = gl.constexpr(
            gl.hcu.AMDWMMALayout(3, transposed=True, warps_per_cta=[4, 1], instr_shape=[16, 16, 32]))
...
class AttentionProgram:
    ...
    k_desc: gl.hcu.gfx1250.tdm.tensor_descriptor
    k_buffer: gl.shared_memory_descriptor

    v_desc: gl.hcu.gfx1250.tdm.tensor_descriptor
    ...
    def initialize(cfg,  #
        ...
        # k [HEAD_SZ, BLOCK_N]
        k_desc = gl.hcu.gfx1250.tdm.make_tensor_descriptor(  #
        ...
        v_desc = gl.hcu.gfx1250.tdm.make_tensor_descriptor(  #
        ...
        q = gl.hcu.gfx1250.buffer_load(q_ptr, q_offs, mask=q_mask)
    ...
    def tdm_shared_load_k(self, buffer_id, wait_count):
        gl.hcu.gfx1250.tdm.async_wait(wait_count)
    ...
    def tdm_load_global_to_shared_k(self, offset, buffer_index):
        gl.hcu.gfx1250.tdm.async_load(self.k_desc, offset, self.k_buffer.index(buffer_index))

    @gluon.jit
    def tdm_load_global_to_shared_v(self, offset, buffer_index):
        gl.hcu.gfx1250.tdm.async_load(self.v_desc, offset, self.v_buffer.index(buffer_index))

    @gluon.jit
    def compute_qk(self, k, cur_seq):
        qk = gl.zeros([self.cfg.BLOCK_M, self.cfg.BLOCK_N], dtype=gl.float32, layout=self.cfg.qk_layout)
        qk = gl.hcu.gfx1250.wmma(self.q, k, qk)
    ...
    def compute_pv(self, p, v, acc):
        ...
        return gl.hcu.gfx1250.wmma(p, v, acc)
    ...
    def store_output(self, out):
        ...
        gl.hcu.gfx1250.buffer_store(casted_out, self.o_ptr, self.o_offs, mask=self.o_mask)
