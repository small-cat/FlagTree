...
def mxgemm_tdm_pipelined_kernel(a_ptr, b_ptr, c_ptr, a_scale, b_scale, M, N, K, stride_am, stride_ak, stride_bk,
    ...
    WMMA_LAYOUT: gl.constexpr = gl.hcu.AMDWMMALayout(3, transposed=True, warps_per_cta=[2, 2],
                                                     instr_shape=[16, 16, 128], tiles_per_warp=tiles_per_warp)
    WMMA_LAYOUT_PACKED: gl.constexpr = gl.hcu.AMDWMMALayout(3, transposed=True, warps_per_cta=[2, 2],
    ...
    A_SCALE_LINEAR_LAYOUT: gl.constexpr = gl.hcu.gfx1250.get_wmma_scale_layout(DOT_LAYOUT_A, [BLOCK_M, BLOCK_K_SCALE])
    B_SCALE_LINEAR_LAYOUT: gl.constexpr = gl.hcu.gfx1250.get_wmma_scale_layout(DOT_LAYOUT_B, [BLOCK_N, BLOCK_K_SCALE])
    ...
    a_desc = gl.hcu.gfx1250.tdm.make_tensor_descriptor(base=a_ptr + pid_m * BLOCK_M * stride_am,
    ...
    if TRANSPOSE_B:
        b_desc = gl.hcu.gfx1250.tdm.make_tensor_descriptor(base=b_ptr + pid_n * BLOCK_N * stride_bn,
    ...
    a_scale_desc = gl.hcu.gfx1250.tdm.make_tensor_descriptor(
    ...
    b_scale_desc = gl.hcu.gfx1250.tdm.make_tensor_descriptor(
    ...
    for _ in gl.static_range(NUM_BUFFERS - 1):
        gl.hcu.gfx1250.tdm.async_load(a_desc, [0, load_idx * BLOCK_K_PACKED_A], a_buffer.index(load_idx))
        if TRANSPOSE_B:
            gl.hcu.gfx1250.tdm.async_load(b_desc, [0, load_idx * BLOCK_K_PACKED_B], b_buffer.index(load_idx))
        else:
            gl.hcu.gfx1250.tdm.async_load(b_desc, [load_idx * BLOCK_K_PACKED_B, 0], b_buffer.index(load_idx))
        gl.hcu.gfx1250.tdm.async_load(a_scale_desc, [0, load_idx * BLOCK_K_SCALE_PRESHUFFLED],
                                      a_scale_buffer.index(load_idx))
        gl.hcu.gfx1250.tdm.async_load(b_scale_desc, [0, load_idx * BLOCK_K_SCALE_PRESHUFFLED],
    ...
    for _ in range(0, gl.cdiv(K, BLOCK_K) - (NUM_BUFFERS - 1)):
        gl.hcu.gfx1250.tdm.async_load(a_desc, [0, load_idx * BLOCK_K_PACKED_A], a_buffer.index(load_idx % NUM_BUFFERS))
        if TRANSPOSE_B:
            gl.hcu.gfx1250.tdm.async_load(b_desc, [0, load_idx * BLOCK_K_PACKED_B],
                                          b_buffer.index(load_idx % NUM_BUFFERS))
        else:
            gl.hcu.gfx1250.tdm.async_load(b_desc, [load_idx * BLOCK_K_PACKED_B, 0],
                                          b_buffer.index(load_idx % NUM_BUFFERS))
        gl.hcu.gfx1250.tdm.async_load(a_scale_desc, [0, load_idx * BLOCK_K_SCALE_PRESHUFFLED],
        ...
        gl.hcu.gfx1250.tdm.async_wait((NUM_BUFFERS - 1) * 4)
        ...
        accumulator = gl.hcu.gfx1250.wmma_scaled(a, scale_a, DTYPE_A, b, scale_b, DTYPE_B, accumulator)
        wmma_idx += 1

    # epilogue
    for i in gl.static_range(NUM_BUFFERS - 1):
        gl.hcu.gfx1250.tdm.async_wait((NUM_BUFFERS - 2 - i) * 4)
        a = a_buffer.index(wmma_idx % NUM_BUFFERS).load(layout=DOT_LAYOUT_A)
        if TRANSPOSE_B:
            b = b_buffer.index(wmma_idx % NUM_BUFFERS).permute([1, 0]).load(layout=DOT_LAYOUT_B)
        else:
