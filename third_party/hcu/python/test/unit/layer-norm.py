...
def _layer_norm_bwd_dx_fused(DX,  # pointer to the input gradient
    ...
    tl.store(DW, partial_dw, mask=mask, cache_modifier=".wt")
    tl.store(DB, partial_db, mask=mask, cache_modifier=".wt")
...
def test_layer_norm(M, N, dtype, eps=1e-5, device='cuda'):
    ...
    atol = 2e-2
    if not torch.allclose(db_tri, db_ref, atol=atol, rtol=0):
        print(f"{db_tri=} {db_ref=}")
        abs_error = torch.abs(db_tri - db_ref)
        # 找到全局最大绝对误差的值和它的扁平化索引
        max_error_value, flat_index = torch.max(abs_error.view(-1), dim=0)
        # 将扁平化索引转换为原始张量形状对应的多维索引
        max_error_index = torch.unravel_index(flat_index, abs_error.shape)
        # 获取该点在两个张量中的具体值
        value_in_tri = db_tri[max_error_index].item()
        value_in_ref = db_ref[max_error_index].item()
        # 打印结果
        print(f"最大绝对误差值: {max_error_value.item():.8f}")
        print(f"误差最大点的坐标: {list(max_error_index)}")
        print(f"db_tri 在该点的值: {value_in_tri:.8f}")
        print(f"db_ref 在该点的值: {value_in_ref:.8f}")
        print(f"实际差值 (db_tri - db_ref): {(value_in_tri - value_in_ref):.8f} > {atol}")
        assert False

    if not torch.allclose(dw_tri, dw_ref, atol=atol, rtol=0):
        print(f"{dw_tri=} {dw_ref=}")
        abs_error = torch.abs(dw_tri - dw_ref)
        # 找到全局最大绝对误差的值和它的扁平化索引
        max_error_value, flat_index = torch.max(abs_error.view(-1), dim=0)
        # 将扁平化索引转换为原始张量形状对应的多维索引
        max_error_index = torch.unravel_index(flat_index, abs_error.shape)
        # 获取该点在两个张量中的具体值
        value_in_tri = dw_tri[max_error_index].item()
        value_in_ref = dw_ref[max_error_index].item()
        # 打印结果
        print(f"最大绝对误差值: {max_error_value.item():.8f}")
        print(f"误差最大点的坐标: {list(max_error_index)}")
        print(f"dw_tri 在该点的值: {value_in_tri:.8f}")
        print(f"dw_ref 在该点的值: {value_in_ref:.8f}")
        print(f"实际差值 (dw_tri - dw_ref): {(value_in_tri - value_in_ref):.8f} > {atol}")
        assert False
